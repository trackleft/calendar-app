...
// function getEvents(){
  ...
  // alert(eventDate);
	...
	// });
  		...
  		if(data.events[d].event.feature=='0'){
  			...
  			eventImage.setAttribute('src','img/'+data.events[d].event.image);
  		...
  		else if(data.events[d].event.feature=='1'){
...
function makeOngoingEvents(){
	console.log('It is making Ongoing Events');
	$.get('json/ongoing.json', function(data){
		console.log(data);
		var ongoingEventsDiv = document.getElementById('ongoing-events-container');console.log(ongoingEventsDiv);
		var slides = document.createElement('ul');
		slides.setAttribute('class','slides');
		var p = 0;console.log(data.events);
		while(p<9){
			...
			ongoingEventsDiv.appendChild(listItemElement);console.log(ongoingEventsDiv);
			p++
		}
	})
}
